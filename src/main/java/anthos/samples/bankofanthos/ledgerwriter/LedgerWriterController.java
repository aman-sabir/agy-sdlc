...
public final class LedgerWriterController {
    ...
    */
    ...
    public LedgerWriterController(
            ...
            @Value("${LOCAL_ROUTING_NUM:123456789}") String localRoutingNum,
            @Value("http://${BALANCES_API_ADDR:localhost:8080}/balances")
                    String balancesApiUri,
            @Value("${VERSION:1.0.0}") String version) {
    ...
    public ResponseEntity<?> addTransaction(
        ...
        try {
            ...
            transactionValidator.validateTransaction(localRoutingNum,
                    transaction.getFromAccountNum(), transaction);
            ...
            if (transaction.getFromRoutingNum().equals(localRoutingNum)) {
                int balance = getAvailableBalance(
                        "mock-token", transaction.getFromAccountNum());
            ...
            return new ResponseEntity<>(savedTx,
                    HttpStatus.CREATED);
